...
if __name__ == "__main__":
   # stuff only to run when not called via 'import' here
    ...
    path = "D:\\Roshinie_105953_Backup\\ADL\\JIRA\\RM Tool Related Docs\\Bulk Update\\allocations_extra_resource_blank.xlsx"
    ...
    for i in range(i_start, i_max):
        ...
        existing_extra_resource_flag = xl_sheet.cell(row=i, column=2).value  # < -- Config

        updated_extra_resource_flag = xl_sheet.cell(row=i, column=3).value  # < -- Config
        ...
        if [ticket_key ] is not None:
            ...
            xl_sheet.cell(row=i, column=3).value = "No"
